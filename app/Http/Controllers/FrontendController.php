...
class FrontendController extends Controller
{
    ...
    public function currencyCalculator(){      
        ...
        $enterAmount=$_POST['fromAmount'];
        $receiveAmountExchangeCharge=0;
        ...
        foreach( $j as $obj ){ 
            if($obj->code==$_POST['fromSymbol']){
                print_r($obj);
                $receiveAmountPrice=$obj->rate;
            break;
            }
        }
        foreach( $j as $obj ){           
            if($obj->code==$_POST['receiveSymbol']){
                $fromAmountPrice=$obj->rate;
            break;
            }
            
        }
       
        if($receiveAmountPrice!=0 and $fromAmountPrice!=0){       
            ...
            $getAmountTotal = ( $fromAmountPrice/$receiveAmountPrice);
            //return $getAmountInput;
            return ['status' => 'success', 'data' => $getAmountInput];
           // return back()->with(['status'=>'success','data' => $getAmountInput]);
        }
        else{
            return ['status' => 'fail', 'data' => $getAmountInput];
        }
    }
}