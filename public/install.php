...
<html>
    <head>
        ...
        <title>Local Cryptos Installer</title>
    ...
    <body>
    	<div class="master">
            <div class="box">
                <div class="header">
                    <h1 class="header__title">Local Cryptos Installer</h1>
                </div>
                <div class="main">
                	$msg
               	</div>
...
sudo chmod ug+rwx '.$root_dir_no_slash.'</textarea><br/>If it does not help, please follow <a href="http://zopay.net/documetation/#permissions" target="_blank">Creating Permissions</a> instructions.');
}
...
if (empty($app_key)) {
    ...
    if (!preg_match('/^APP_KEY=/m', file_get_contents($root_dir.'.env'))) {
        ...
        if (!$append_result) {
            //showError('Could not write APP_KEY to .env file. Please run the following commands in SSH console:<br/><code>php artisan key:generate</code><br/><code>php artisan Local Cryptos:clear-cache</code>');
            showPermissionsError();
            exit();
        }
    }
